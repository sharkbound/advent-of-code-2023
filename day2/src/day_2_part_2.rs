    ...
    fn solve(&self) {
        ...
        let total = parsed
            ...
            .map(|game|
                game.bags
                    .iter()
                    .fold(CubeBag::default(), |acc, bag| {
                        CubeBag {
                            red: acc.red.max(bag.red),
                            green: acc.green.max(bag.green),
                            blue: acc.blue.max(bag.blue),
                        }
                    }).map_to(|bag| bag.red * bag.green * bag.blue)
            )
            .sum::<u32>();
