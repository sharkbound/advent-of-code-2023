...
impl daytemplate::Day for Day3Part2 {
    ...
    fn solve(&self) {
        ...
        let mut total = 0u32;
        for (symbol, index) in processed.symbols.iter() {
            if symbol != &'*' {
                continue;
            }

            let numbers = find_numbers_around_index(&numbers_array, index).iter().map(|x| *x).collect_to_vec();
            if numbers.len() != 2 {
                continue;
            }

            total += numbers.first().unwrap() * numbers.last().unwrap();
        }
